...
test('deletes an asset with confirmation', async ({ page }) => {
  ...
  await expect(
    page.getByText('Are you sure you want to delete "AAPL"'),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Delete' }).click();
...
test('deactivating one of multiple assets does not show warning', async ({
  page,
}) => {
  await createGroup(page, { name: 'Stocks', target: '60', deviation: '5' });
  await addAsset(page, 'Stocks', { name: 'AAPL', type: 'unit' });
...
test('deleting a group removes its assets from calculator', async ({
  page,
}) => {
  await createGroup(page, { name: 'Stocks', target: '60', deviation: '5' });
  ...
  await expect(
    page.getByRole('paragraph').filter({ hasText: 'Treasury Fund' }),
  ).toBeVisible();
