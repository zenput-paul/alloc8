...
export function useCalculator() {
  ...
  const { result: assets, isFetching: assetsFetching } =
    useRxQuery(assetsQuery);

  const [assetInputs, setAssetInputs] = useState<
    Record<string, AssetFormValues>
  >({});
  ...
  const isValid =
    percentagesValid && allGroupsHaveActiveAssets && totalInvestment !== '';
  ...
  function handleCalculate() {
    ...
    try {
      ...
      setError(
        t('calculator.calculationError', { message: (err as Error).message }),
      );
    }
  }
