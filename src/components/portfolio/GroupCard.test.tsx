...
const defaultGroup = { id: 'g1', name: 'Stocks', targetPercentage: 60, deviationThreshold: 5 }

describe('GroupCard', () => {
  it('renders group name and target info', () => {
    render(<GroupCard group={defaultGroup} onEdit={() => {}} onDelete={() => {}} />)
  ...
  it('starts expanded when defaultExpanded is true', () => {
    ...
    render(
      <GroupCard group={defaultGroup} onEdit={() => {}} onDelete={() => {}} defaultExpanded />,
    )

    expect(screen.getByText('1 asset')).toBeInTheDocument()
    expect(screen.getByText('AAPL')).toBeInTheDocument()
    expect(screen.getByText('Add Asset')).toBeInTheDocument()
    expect(screen.queryByText(/won't receive new investments/)).not.toBeInTheDocument()
  })
  ...
  it('shows assets when expanded', async () => {
    ...
    setMockAssets([
      { id: 'a1', groupId: 'g1', name: 'AAPL', type: 'unit', active: true },
      { id: 'a2', groupId: 'g1', name: 'Bond Fund', type: 'fixed', active: false },
    ])
    render(<GroupCard group={defaultGroup} onEdit={() => {}} onDelete={() => {}} />)

    // Expand the card
    const expandButtons = screen.getAllByRole('button')
    const expandButton = expandButtons.find(b => b.querySelector('[data-testid="ExpandMoreIcon"]'))!
    ...
    expect(screen.getByText(/won't receive new investments/)).toBeInTheDocument()
  })
  ...
  it('shows "No assets" when expanded with empty assets', () => {
    ...
    expect(screen.getByText('No assets yet — add one to get started')).toBeInTheDocument()
  })

  it('calls onEdit when edit menu item is clicked', async () => {
    const user = userEvent.setup()
    const onEdit = vi.fn()
    render(<GroupCard group={defaultGroup} onEdit={onEdit} onDelete={() => {}} />)

    // Open the menu
    await user.click(screen.getAllByRole('button').find(b => b.querySelector('[data-testid="MoreVertIcon"]'))!)
  ...
  it('calls onDelete when delete menu item is clicked', async () => {
    ...
    render(<GroupCard group={defaultGroup} onEdit={() => {}} onDelete={onDelete} />)
