...
describe('AssetDialog', () => {
  it('renders add mode with empty name and unit type selected', () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Units', pressed: true }),
    ).toBeInTheDocument();
  });
  ...
  it('renders edit mode with pre-filled fields', () => {
    ...
    render(
      <AssetDialog open onClose={() => {}} groupId="g1" editItem={asset} />,
    );
  ...
  it('patches an existing asset in edit mode', async () => {
    ...
    render(
      <AssetDialog open onClose={onClose} groupId="g1" editItem={asset} />,
    );
