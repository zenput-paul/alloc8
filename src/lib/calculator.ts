import type { Group, Asset, AssetInput, AssetAllocation, GroupStats } from '../types'
...
function groupPct(ctx: GroupContext, groupId: string, extraAllocation = 0): number {
  return ((ctx.groupCurrentValue.get(groupId)! + extraAllocation) / ctx.futureTotal) * 100
}
...
function allocateToBelowRangeGroups(
  ...
  const belowRange = ctx.groups.filter(g =>
    groupPct(ctx, g.id) < g.targetPercentage - g.deviationThreshold,
  )
  ...
  for (const group of belowRange) {
    ...
    const maxAllocation = Math.max(0, targetValue - ctx.groupCurrentValue.get(group.id)!)
    groupAllocation.set(group.id, Math.min(share, maxAllocation))
  }

  return remaining - Array.from(groupAllocation.values()).reduce((s, v) => s + v, 0)
}
...
function distributeToEligibleGroups(
  ...
  const eligible = ctx.groups.filter(g => {
    const totalValue = ctx.groupCurrentValue.get(g.id)! + groupAllocation.get(g.id)!
...
function capAndRebalance(
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (const group of ctx.groups) {
      const totalValue = ctx.groupCurrentValue.get(group.id)! + groupAllocation.get(group.id)!
      const pct = (totalValue / ctx.futureTotal) * 100
      ...
      if (pct > cap && groupAllocation.get(group.id)! > 0) {
        ...
        const maxAlloc = Math.max(0, capValue - ctx.groupCurrentValue.get(group.id)!)
    ...
    const underCap = ctx.groups.filter(g =>
      !capped.includes(g.id) &&
      (ctx.groupActiveAssets.get(g.id)?.length ?? 0) > 0 &&
      ((ctx.groupCurrentValue.get(g.id)! + groupAllocation.get(g.id)!) / ctx.futureTotal) * 100 <
  ...
  while (changed && totalRemainder > 0) {
    ...
    const unitAssets = allocations
      ...
      .map(a => {
        ...
        const groupValue = ctx.groupCurrentValue.get(group.id)! + groupAllocation.get(group.id)!
        const gap = group.targetPercentage - (groupValue / ctx.futureTotal) * 100
...
function distributeRemainderToFixedAssets(
  ...
  const eligibleGroups = ctx.groups.filter(g => {
    ...
    const groupValue = ctx.groupCurrentValue.get(g.id)! + (allocatedByGroup.get(g.id) ?? 0)
  ...
  const totalTarget = eligibleGroups.reduce((s, g) => s + g.targetPercentage, 0)
...
function computeGroupStats(
  ...
  const totalCurrentValue = Array.from(ctx.groupCurrentValue.values()).reduce((s, v) => s + v, 0)
  ...
  for (const group of ctx.groups) {
    ...
    stats.set(group.id, {
      currentPct: totalCurrentValue > 0 ? (currentVal / totalCurrentValue) * 100 : 0,
      afterPct: totalAfterValue > 0 ? ((currentVal + invested) / totalAfterValue) * 100 : 0,
    })
  }
...
export function calculateAllocations(
  ...
  const { allocations, totalRemainder } = distributeToAssets(ctx, groupAllocation, remaining)
  let remainder = reinvestRemainderInUnits(ctx, groupAllocation, allocations, totalRemainder)
